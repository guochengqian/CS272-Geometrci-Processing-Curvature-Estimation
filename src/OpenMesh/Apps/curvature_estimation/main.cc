...
int main(int argc, char *argv[]) {
    ...
    //// --------------------- read meashes
    _Mesh mesh;
    if (!OpenMesh::IO::read_mesh(mesh, "/home/qiang/Documents/codefiles/geo/model/Op_BL3_trim_Fine.obj")) {
        std::cerr << "read error\n";
        exit(1);
    }
...
//    view_f_normals(mesh, f_normals, f_centers);
    ...
    //// ----------------- optimization, update the vertices normals
...
//    lin_sys_b << 2.0 ,  2.0 ,  2.0 ,  0 ,  2.0 ,  2.0 ,  2.0 ,  2.0, 0.0, 0.0, 0.0, 0.0;
    ...
    //// optimization
    //    for (int k =0; k < 1; k++) {
    for (int k = 0; k < atoi(argv[1]); k++) {   // optimizaiton steps
